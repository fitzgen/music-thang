...
/***/ function(module, exports, __webpack_require__) {
	...
	var makeFlexBoxClass = function makeFlexBoxClass(direction) {
	...
	var vbox = makeFlexBoxClass("column");
	var hbox = makeFlexBoxClass("row");

	var TimeVizCell = React.createClass({
	  displayName: "TimeVizCell",

	  render: function render() {
	    var className = "time-viz-cell";
	    if (this.props.on) {
	      className += " on";
	    }
	...
	var TimeViz = React.createClass({
	  ...
	  render: function render() {
	    ...
	    return vbox({
	      className: "time-viz"
	    }, this.props.noteTypes.map(function (nt, idx) {
	      return TimeVizCell({
	        key: "time-viz-cell-" + nt.name,
	        noteType: nt,
	        on: _this.props.noteIndex % nt.mod === 0
	      });
	    }));
	  }
	});
	...
	var PadsColumn = React.createClass({
	  ...
	  render: function render() {
	    ...
	    return vbox({
	      flex: 1
	    }, this.props.sound.keys.map(function (k, i) {
	      return Pad({
	        key: "key-" + k,
	        padKey: k,
	        sound: _this2.props.sound,
	        noteType: i,
	        noteTypes: _this2.props.noteTypes,
	        scheduleSound: _this2.props.scheduleSound,
	        unscheduleSound: _this2.props.unscheduleSound
	      });
	    }));
	  }
	});
	...
	var Pads = React.createClass({
	  ...
	  render: function render() {
	    ...
	    return hbox({
	      flex: 1
	    }, this.props.sounds.map(function (s) {
	      return PadsColumn({
	        key: "pads-column-" + s.name,
	        sound: s,
	        noteTypes: _this3.props.noteTypes,
	        scheduleSound: _this3.props.scheduleSound,
	        unscheduleSound: _this3.props.unscheduleSound
	      });
	    }));
	  }
	});

	var SoundsHeaderCell = React.createClass({
	  displayName: "SoundsHeaderCell",

	  render: function render() {
	    var className = "sounds-header-cell";
	    if (this.props.on) {
	      className += " on";
	    }
	...
	var SoundsHeader = React.createClass({
	  ...
	  render: function render() {
	    ...
	    return hbox({
	      flex: 1
	    }, this.props.sounds.map(function (s, idx) {
	      return SoundsHeaderCell({
	        key: "sounds-header-cell-" + s.name,
	        sound: s,
	        on: _this4.props.noteTypes.some(function (nt) {
	          return nt.scheduled.contains(s.name) && _this4.props.noteIndex % nt.mod === 0;
	        })
	      });
	    }));
	  }
	...
	function whenSoundKeys(listener) {
	  return function (e) {
	    ...
	    var res = this._getKeySoundType(e.key.toUpperCase());
	    if (!res) {
	      return;
	    }
	...
	var BPMControls = React.createClass({
	  displayName: "BPMControls",

	  render: function render() {
	    return hbox({
	      className: "bpm"
	    }, dom.label(null, "BPM: " + this.props.bpm), dom.input({
	      ref: "range",
	      type: "range",
	      min: 50,
	      max: 150,
	...
	var Thang = React.createClass({
	  ...
	  shouldComponentUpdate: function shouldComponentUpdate(nextProps) {
	    if (this.props.noteIndex !== nextProps.noteIndex || this.props.bpm !== nextProps.bpm) {
	      return true;
	    }

	    for (var i = 0; i < this.props.noteTypes.length; i++) {
	  ...
	  render: function render() {
	    ...
	    }, "Latency: " + this.props.latency + "ms"), BPMControls({
	      bpm: this.props.bpm,
	      setBPM: this._setBPM
	    }), hbox(null, dom.div({
	      className: "spacer"
	  ...
	  _setBPM: function _setBPM(bpm) {
	    this.setProps({ bpm: bpm });
	  },

	  _getKeySoundType: function _getKeySoundType(key) {
	    var _iteratorNormalCompletion = true;
	    var _didIteratorError = false;
	    var _iteratorError = undefined;

	    try {
	      for (var _iterator = this.props.sounds[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
	        var s = _step.value;

	        var idx = s.keys.indexOf(key);
	        if (idx >= 0) {
	          return {
	            sound: s,
	            noteTypeIndex: idx
	  ...
	  _onKeyDown: whenSoundKeys(function (_ref) {
	    var sound = _ref.sound;
	    var noteTypeIndex = _ref.noteTypeIndex;

	    this._scheduleSound(sound, noteTypeIndex);
	  }),

	  _onKeyUp: whenSoundKeys(function (_ref2) {
	    var sound = _ref2.sound;
	    var noteTypeIndex = _ref2.noteTypeIndex;

	    this._unscheduleSound(sound, noteTypeIndex);
	  }),

	  _scheduleSound: function _scheduleSound(sound, noteTypeIndex) {
	    this.setProps({
	      noteTypes: this.props.noteTypes.map(function (nt, idx) {
	        if (idx !== noteTypeIndex) {
	          return nt;
	        }

	        nt.scheduled = nt.scheduled.filter(function (n) {
	          return n !== sound.name;
	        }).concat([sound.name]);
	        return nt;
	      })
	  ...
	  _unscheduleSound: function _unscheduleSound(sound, noteTypeIndex) {
	    this.setProps({
	      noteTypes: this.props.noteTypes.map(function (nt, idx) {
	        if (idx !== noteTypeIndex) {
	          return nt;
	...
	function loadBuffers(component, context) {
	  ...
	  try {
	    var _loop = function _loop() {
	      ...
	      var sound = s; // Temporary work around SpiderMonkey bug.
	      var xhr = new XMLHttpRequest();
	      xhr.open("GET", sound.url, true);
	      xhr.responseType = "arraybuffer";

	      var errorHandler = function errorHandler(prefix) {
	    ...
	    for (var _iterator2 = component.props.sounds[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
	      _loop();
	    }
	  } catch (err) {
	...
	function loop(component, context) {
	  ...
	  var scheduleNextLoop = function scheduleNextLoop() {
	    lastLoopTime = now;
	    setTimeout(loop, 1, component, context);
	  };

	  var interval = getShortestInterval(component.props.bpm);
	  var lastTime = component.props.lastNoteTime;
	  var elapsed = now - lastTime;
	  var notesHappened = elapsed / interval | 0;

	  if (!notesHappened) {
	    return void scheduleNextLoop();
	  }

	  var currentNoteIndex = (component.props.noteIndex + notesHappened) % 8;
	  ...
	  try {
	    for (var _iterator3 = component.props.noteTypes[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
	      var noteType = _step3.value;
	      ...
	      try {
	        var _loop2 = function _loop2() {
	          ...
	          var sound = component.props.sounds.find(function (s) {
	            return s.name === name;
	          });
	          playSound(context, sound.buffer);
	        };

	        for (var _iterator4 = noteType.scheduled[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
	          _loop2();
	        }
	      } catch (err) {
